...
Reconcile reviewed `infra/` changes before rolling out an application image. The Python runtime is qualified for the declared concurrency of **8** and the startup probe's **6 attempts at 5-second intervals** within the 256 MiB service limit. CI and the manual deploy task change only the image; merging a branch does not apply service settings. After each image rollout, verify health, traffic, error logs, and browser journeys; keep the previous known-good digest available for rollback.
