...
The top navigation keeps Portfolio, Articles, and Sites visible on every screen. The portfolio section rail appears from 1600px, where the content margin accommodates its 192px width without covering text; smaller screens use a native section dropdown. Section links work without JavaScript; JavaScript adds the active-section indicator, closes the dropdown after selection, and supports Escape. Social profile icons stay in the header at every width. The header stays on one row, and the Fmind.dev wordmark appears when its column has enough room. Current consulting availability and the paid mentoring duration appear beside the homepage contact actions and derive from the same service data as the Services section.
